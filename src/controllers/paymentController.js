...
function initPaymentRoutes(app) {
  ...
  app.post('/payment', async (req, res) => {
    const { tour_id, user_id, adults, children, total_price, service_date } = req.body;
    ...
    try {
      ...
      if (result.data.return_code === 1) {
        ...
        res.status(400).json({ message: 'ZaloPay API error', details: result.data });
      }
    } catch (error) {
  ...
  app.post('/callback', async (req, res) => {
    ...
    try {
      ...
      if (reqMac !== mac) {
        ...
        console.log("Determined transaction status from checkOrderStatus:", status);
